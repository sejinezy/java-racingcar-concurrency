...
본 프로젝트는 전략 시뮬레이션(StrategyAi)을 추가했기 때문에 각 자동차가 매 라운드마다 내부적으로 2000회 시뮬레이션을 수행하여 최적 전략을 결정합니다.
...
---
### 🖥 환경에 따른 성능 차이
같은 코드, 같은 입력값이라도 **실행 환경(CPU,코어 수)**에 따라 멀티스레드의 효과가 얼마나 달라지는지 확인해보기위해 노트북과 데스크탑에서 동일한 벤치마크를 돌려보았습니다.

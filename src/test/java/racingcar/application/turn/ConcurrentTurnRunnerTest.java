...
class ConcurrentTurnRunnerTest {
    ...
    void run_한_턴_실행시_참여한_자동차_수만큼_결과를_반환한다() {
        ParticipatingCars cars = new ParticipatingCars(List.of("pobi", "woni", "jun"));
        Car pobi = cars.getCars().getFirst();
        Car woni = cars.getCars().get(1);
        Car jun = cars.getCars().get(2);
        FixedStrategyAi ai = new FixedStrategyAi(new DefaultWinRateCalculator(10));
        ConcurrentTurnRunner runner = new ConcurrentTurnRunner(ai, cars, es);
        int remainTurns = 3;
        ...
        assertThat(result.positions())
                .anySatisfy(p -> {
                    if (p.name().equals("jun")) {
                        assertThat(p.position()).isEqualTo(jun.getPosition());
                    }
                });
    }
}